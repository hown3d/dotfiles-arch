...
active_display = subprocess.check_output('$HOME/.config/wofi/scripts/get_active_display.sh', shell=True).decode().strip()


# Call rofi and move focus to the selected window
selection = subprocess.check_output(['wofi', '--dmenu','--i', '--monitor', 'XWAYLAND' + active_display ], input=windows_string, universal_newlines=True)
